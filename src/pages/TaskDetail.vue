  ...
  <div class="min-h-screen p-6 bg-gray-50">
    <div class="max-w-3xl mx-auto">
      ...
      <div v-else-if="error" class="p-6 bg-red-50 rounded shadow">
        ...
        <div class="mt-4">
          <button @click="reload" class="px-4 py-2 bg-blue-600 text-white rounded">Retry</button>
        </div>
      </div>
      ...
      <div v-else-if="task" class="bg-white rounded-lg shadow p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <!-- Image -->
          <div class="w-full h-48 md:h-auto md:col-span-1 rounded overflow-hidden bg-gray-100 flex items-center justify-center">
            <img
              ...
              @error="onImageError"
            />
            <div v-if="(!task.image_url || imageErrored) && !imageLoaded" class="text-gray-500">
              No Image
            </div>
          </div>
          ...
          <div class="md:col-span-2 flex flex-col gap-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div class="flex-1">
                ...
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  >
                    <img v-if="category.icon_url" :src="category.icon_url" class="w-4 h-4" />
                  ...
                  <span class="text-sm text-gray-500">
                    Priority: <strong class="ml-1">{{ task.priority || 'medium' }}</strong>
                  </span>
              ...
              <div class="text-right mt-4 md:mt-0 flex flex-col md:items-end gap-2">
                <button
                ...
                <div class="flex justify-end gap-2 mt-2 md:mt-3 flex-wrap">
                  <button @click="openEdit" class="px-3 py-1 bg-[#825b64] text-white rounded hover:bg-[#69474f]">Edit</button>
                  <button @click="confirmDelete" class="px-3 py-1 bg-red-700 text-white rounded hover:bg-red-800">Delete</button>
                </div>
              </div>
            </div>
            ...
            <div class="text-sm text-gray-500 mt-2 md:mt-4">
              ...
              <div v-if="task.updated_at">Updated: {{ formatDate(task.updated_at) }}</div>
            </div>
          </div>
        </div>
      </div>
...
const category = computed(() => categoryStore.categories.find(c => c.id === task?.category_id) || null);
...
async function toggleComplete() {
  try {
    await taskStore.editTask(task.value.id, { completed: !task.value.completed });
